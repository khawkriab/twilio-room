...
function AudioLevelIndicator({ size, audioTrack, background }: { size?: number; audioTrack?: AudioTrack; background?: string }) {
    const SIZE = size || 24
    ...
    return isTrackEnabled ? (
        ...
        <MicOff height={`${SIZE}px`} width={`${SIZE}px`} style={{ width: 'initial', height: 'initial' }} data-cy-audio-mute-icon />
    )
}
